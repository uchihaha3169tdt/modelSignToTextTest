# %%writefile /kaggle/working/mdm/diffusion/gaussian_diffusion.py
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.l1_loss = lambda a, b: th.abs(a - b)  # th.nn.MSELoss(reduction='none')  # must be None for handling mask later on.
        self.smooth_l1_loss = lambda a, b: th.where(
            th.abs(a - b) < 1,
            0.5 * (a - b)**2,
            th.abs(a - b) - 0.5
        )
    ...
    def masked_smoothl1(self, a, b, mask):
        # assuming a.shape == b.shape == bs, J, Jdim, seqlen
        # assuming mask.shape == bs, 1, 1, seqlen
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, dataset=None):
        ...
        m_len = model_kwargs['y']['lengths'].int().to(x_start.device)
        max_len = x_start.shape[-1]
        lv_mask = self.create_loss_mask(m_len, max_len=max_len).unsqueeze(1).unsqueeze(1).to(x_start.device)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            model_output = model_output * lv_mask # 150
            target = target * lv_mask # 151
            
            terms["rot_smooth_mae"] = self.masked_smoothl1(target, model_output, mask) # mean_flat(rot_mse)

            if self.lambda_vel > 0.:
                target_vel = (target[..., 1:] - target[..., :-1])
                model_output_vel = (model_output[..., 1:] - model_output[..., :-1])
                terms["vel_smooth_mae"] = self.masked_smoothl1(target_vel[:, :-1, :, :], # Remove last joint, is the root location!
                                                  model_output_vel[:, :-1, :, :],
                                                  mask[:, :, :, 1:])  # mean_flat((target_vel - model_output_vel) ** 2)

            terms["loss"] = terms["rot_smooth_mae"] + terms.get('vb', 0.) +\
                            (self.lambda_vel * terms.get('vel_smooth_mae', 0.)) +\
                            (self.lambda_rcxyz * terms.get('rcxyz_smooth_mae', 0.)) + \
